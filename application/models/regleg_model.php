...
class Regleg_model extends CI_Model
{
     ...
     */
    function DaftarRegLegCount($searchText = '', $dt1 , $dt2, $jenisdok)
    {
        ...
        if(!empty($jenisdok)) {
            $likeCriteria = "rlg.jenisdokId='".$jenisdok."'";
            $this->db->where($likeCriteria);
        }
    ...
    function DaftarRegLeg($searchText = '', $dt1 , $dt2, $jenisdok, $page, $segment)
    {
        $this->db->select('*,  jdk.nama as nmjdk , pjb.nama as nmpjb, rlg.tanggal as rlgtgl');
        $this->db->from('tbl_regleg as rlg');
        $this->db->join('tbl_pejabat as pjb', 'pjb.pejabatId = rlg.pejabatId');
    ...
    function cetakBasedPost($searchText = '', $dt1 , $dt2, $jenisdok)
    {
        $this->db->select('*,  jdk.nama as nmjdk , pjb.nama as nmpjb, rlg.tanggal as rlgtgl');
        $this->db->from('tbl_regleg as rlg');
        $this->db->join('tbl_pejabat as pjb', 'pjb.pejabatId = rlg.pejabatId');
    ...
    */
    ...
    function getreglegInfoByNIK($nik)
    {
        $this->db->select('*');
        $this->db->from('tbl_regleg');

...
-- Dumping structure for table cias.tbl_status
DROP TABLE IF EXISTS `tbl_status`;
CREATE TABLE IF NOT EXISTS `tbl_status` (
  `statusId` tinyint(4) NOT NULL AUTO_INCREMENT COMMENT 'status id',
  `status` varchar(50) NOT NULL COMMENT 'status text',
  PRIMARY KEY (`statusId`)
) ENGINE=MyISAM AUTO_INCREMENT=4 DEFAULT CHARSET=utf8;

-- Dumping data for table cias.tbl_status: 3 rows
/*!40000 ALTER TABLE `tbl_status` DISABLE KEYS */;
REPLACE INTO `tbl_status` (`statusId`, `status`) VALUES
...
REPLACE INTO `tbl_users` (`userId`, `email`, `password`, `name`, `mobile`, `statusId`, `isDeleted`, `createdBy`, `createdDtm`, `updatedBy`, `updatedDtm`) VALUES
